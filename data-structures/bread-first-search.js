...
var doBFS = function(graph, source) {
    ...
    while(!queue.isEmpty()) {
        ...
        var neighbors = graph[currentVertice];
        for(var j=0; j< neighbors.length; j++){
            var currentNeighbor = graph[currentVertice][j];
            if(bfsInfo[currentNeighbor].distance === null){
                bfsInfo[currentNeighbor].distance = bfsInfo[currentVertice].distance + 1;
                bfsInfo[currentNeighbor].predecessor = currentVertice;
